...
local colors = vim.tbl_deep_extend('force', DEFAULT_COLORS, vim.g.om_colors or {})
...
M.apply = function()
    ...
    -- local isItalic = vim.g.om_italic_comment == true
	...
	if(vim.g.om_transparent_bg == true) then
	...
	if(vim.g.om_show_end_of_buffer == true) then
	...
	highlight("TSConstant", colors.purple, nil, "italic", nil)
	highlight("TSConstantBuiltin", colors.purple, nil, "italic", nil)
